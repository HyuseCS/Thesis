...
1. Put your thesis papers in a Zotero collection named **Thesis** (or another name, see step 3). Make sure file sync is on so the PDFs are uploaded to zotero.org.
2. Create a **read-only** API key at <https://www.zotero.org/settings/keys> (New Private Key → allow library read access). The same page shows your numeric **userID**.
3. Create the config file `~/.config/thesis-zotero.env` (outside the repo — the repo is public, never commit the key):
   ...
   Then restrict it: `chmod 600 ~/.config/thesis-zotero.env`
